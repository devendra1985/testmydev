...
@Service
public class SamlUserService implements ApplicationListener<AuthenticationSuccessEvent> {
    ...
    public void onApplicationEvent(AuthenticationSuccessEvent event) {
        if (!(event.getAuthentication().getPrincipal() instanceof Saml2AuthenticatedPrincipal principal)) {
            return;
        }
        provisionUser(principal);
    }

    public void provisionUser(Saml2AuthenticatedPrincipal principal) {
        String nameId = principal.getName();
        String registrationId = principal.getRelyingPartyRegistrationId();

        // Already exists — nothing to do
        if (userRepository.findBySamlNameId(nameId).isPresent()) {
            log.info("SAML login for existing user with nameId: {}", nameId);
            return;
        }
        ...
        log.info("Auto-provisioned new SAML user: {}", username);
    }

    private String getFirstAttribute(Saml2AuthenticatedPrincipal principal, String... names) {
